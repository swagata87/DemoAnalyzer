...
#ifndef localCovPhiPhi_h
#define localCovPhiPhi_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   localCovPhiPhi(TTree *tree=0);
   virtual ~localCovPhiPhi();
...
#ifdef localCovPhiPhi_cxx
localCovPhiPhi::localCovPhiPhi(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/eos/cms/store/group/phys_egamma/swmukher/Run3_Redef_Nov30/all_2024_small.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/eos/cms/store/group/phys_egamma/swmukher/Run3_Redef_Nov30/all_2024_small.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/eos/cms/store/group/phys_egamma/swmukher/Run3_Redef_Nov30/all_2024_small.root:/demo");
...
localCovPhiPhi::~localCovPhiPhi()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t localCovPhiPhi::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t localCovPhiPhi::LoadTree(Long64_t entry)
{
...
void localCovPhiPhi::Init(TTree *tree)
{
...
void localCovPhiPhi::Show(Long64_t entry)
{
...
Int_t localCovPhiPhi::Cut(Long64_t entry)
{
...
#endif // #ifdef localCovPhiPhi_cxx
