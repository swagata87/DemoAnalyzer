...
#ifndef localCovEtaPhi_EE_h
#define localCovEtaPhi_EE_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   localCovEtaPhi_EE(TTree *tree=0);
   virtual ~localCovEtaPhi_EE();
...
#ifdef localCovEtaPhi_EE_cxx
localCovEtaPhi_EE::localCovEtaPhi_EE(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/eos/cms/store/group/phys_egamma/swmukher/Run3_Redef_Nov30/all_2024_medium.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/eos/cms/store/group/phys_egamma/swmukher/Run3_Redef_Nov30/all_2024_medium.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/eos/cms/store/group/phys_egamma/swmukher/Run3_Redef_Nov30/all_2024_medium.root:/demo");
...
localCovEtaPhi_EE::~localCovEtaPhi_EE()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t localCovEtaPhi_EE::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t localCovEtaPhi_EE::LoadTree(Long64_t entry)
{
...
void localCovEtaPhi_EE::Init(TTree *tree)
{
...
Bool_t localCovEtaPhi_EE::Notify()
{
...
void localCovEtaPhi_EE::Show(Long64_t entry)
{
...
Int_t localCovEtaPhi_EE::Cut(Long64_t entry)
{
...
#endif // #ifdef localCovEtaPhi_EE_cxx
